...
public class TestJob implements BaseJob {
    ...
    public void execute(JobExecutionContext context) {
        log.info("Update过期订单 Status  开始执行 Time :{}", DateUtil.now());
        Date date2 = com.matariky.utils.DateUtil.getEndTimeOfDay(new Date());
        String sql1 = "update order_info as t set t.order_status = '3' where t.delete_time = 0 and t.order_status != '3' and t.expiration_end_time <= "+date2.getTime();
