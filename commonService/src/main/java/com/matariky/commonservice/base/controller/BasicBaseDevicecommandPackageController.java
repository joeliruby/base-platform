...
public class BasicBaseDevicecommandPackageController {
	...
	public Object daclist(HttpServletRequest request,@ApiIgnore @RequestParam Map<String, Object> params,	@ApiParam(value = " Tenant ID", required = true) @PathVariable("tenantId") String tenantId,	@ApiParam(value = "JWT Token", required = true) @RequestHeader("Authorization") String jwt) {
	 ...
	 if (dict == null) {
		...
		List<BasicBaseDevicecommandPackage> commonDictList =basicBaseDevicecommandPackageService.getBasicBaseDevicecommandPackageDAC(params, request);
	...
	public Object save(@RequestBody BasicBaseDevicecommandPackage bean,HttpServletRequest request, HttpServletResponse response) {
		try{
			int success = basicBaseDevicecommandPackageService.createBasicBaseDevicecommandPackageWithOrg(bean, request);
				if(success > 0){
	...
	public Object update(@RequestBody BasicBaseDevicecommandPackage bean,HttpServletRequest request, HttpServletResponse response) {
		try{
	...
	public Object getOne(@PathVariable("/basicBaseDevicecommandPackageId") Long id, HttpServletRequest request, HttpServletResponse response) {
			 return new AjaxResult(HttpStatus.OK.value(),AjaxResult.SUCCESS,basicBaseDevicecommandPackageService.selectById(id));
}
