...
public class BasicBaseDeviceCorrelationController {
	...
	public Object list(HttpServletRequest request,BasicBaseDeviceCorrelation bean, @ApiParam(value = " Tenant ID", required = true) @PathVariable("tenantId") String tenantId, @ApiParam(value = "Page Index", required = true) @RequestParam("index") int pageIndex,@ApiParam(value = "Page Size", required = true) @RequestParam("perPage") int perPage, @ApiParam(value = "JWT Token", required = true) @RequestHeader("Authorization") String jwt) {
		PageHelper.startPage(pageIndex, perPage);
		PageInfo<BasicBaseDeviceCorrelation> page = new PageInfo( basicBaseDeviceCorrelationService.getBasicBaseDeviceCorrelationAll(bean));
	...
	public Object daclist(HttpServletRequest request,@ApiIgnore @RequestParam Map<String, Object> params,	@ApiParam(value = " Tenant ID", required = true) @PathVariable("tenantId") String tenantId,	@ApiParam(value = "JWT Token", required = true) @RequestHeader("Authorization") String jwt) {
	 ...
	 if (dict == null) {
		...
		List<BasicBaseDeviceCorrelation> commonDictList =basicBaseDeviceCorrelationService.getBasicBaseDeviceCorrelationDAC(params, request);
	...
	public Object save(@RequestBody BasicBaseDeviceCorrelation bean,HttpServletRequest request, HttpServletResponse response) {
		try{
	...
	public Object update(@RequestBody BasicBaseDeviceCorrelation bean,HttpServletRequest request, HttpServletResponse response) {
		try{
	...
	public Object getOne(@PathVariable("/basicBaseDeviceCorrelationId") Long id, HttpServletRequest request, HttpServletResponse response) {
			 return new AjaxResult(HttpStatus.OK.value(),AjaxResult.SUCCESS,basicBaseDeviceCorrelationService.selectById(id));
}
