...
public class BasicBaseAntifakeController {
    ...
    public AjaxResult list(BasicBaseAntifake bean,
        ...
        PageInfo<BasicBaseAntifake> page = new PageInfo<BasicBaseAntifake>(
                basicBaseAntifakeService.getBasicBaseAntifakeAll(bean, pageIndex, perPage));
    ...
    public String authenticate2(
            @RequestParam(value = "samemasterkey", required = false, defaultValue = "false") boolean samemasterkey,
            @RequestParam(value = "checkforshorttid", required = false, defaultValue = "false") boolean checkforshorttid,
        ...
        if (!StringUtils.hasLength(keycloakToken))
            throw new QslException(MessageKey.KEYCLOAK_TOKEN_NOT_EXIST);
        try (CloseableHttpClient httpclient = HttpClients.createDefault()) {
            // Prepare a request object
            String authUrl = iotUrl + "/v0/authenticate?samemasterkey=" + samemasterkey + "&checkforshorttid="
                    + checkforshorttid;
