...
public interface CommonDictTypeMapper extends BaseMapper<CommonDictType> {
	...
	// According to Dictionary group's key Query all Dictionary Detail under it
	...
	@Override
	int update(@Param(Constants.ENTITY) CommonDictType entity,
			@Param(Constants.WRAPPER) Wrapper<CommonDictType> updateWrapper);
	...
	Page<CommonDictType> selectPage(Page<CommonDictType> page,
			@Param(Constants.WRAPPER) Wrapper<CommonDictType> queryWrapper);

	Page<Map<String, Object>> selectMapsPage(Page<CommonDictType> page,
			@Param(Constants.WRAPPER) Wrapper<CommonDictType> queryWrapper);

	public int updateDeleteTimeById(@Param("array") String[] id);

	public CommonDictType getDictTypeByKey(@Param("tenantId") String tenantId,
			@Param("dictTypeKey") String dictTypeKey);
