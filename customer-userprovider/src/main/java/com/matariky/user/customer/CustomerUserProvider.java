...
import com.matariky.user.customer.external.CredentialData;
import com.matariky.user.customer.external.Customer;
import com.matariky.user.customer.external.CustomerClient;
import com.matariky.user.customer.external.CustomerClientSimpleHttp;
 ...
 */
@Slf4j
public class CustomerUserProvider implements UserStorageProvider,
	...
	public CustomerUserProvider(KeycloakSession session, ComponentModel model) {
		this.session = session;
		this.model = model;
		this.client = new CustomerClientSimpleHttp(session, model);
	}
	...
	public boolean isValid(RealmModel realm, UserModel user, CredentialInput input) {
		...
		if(passwordHashProvider==null) {
			passwordHashProvider=new YXPWDHashProvider();
		}
	...
	private UserModel findUser(RealmModel realm, String identifier) {
		...
		if (adapter == null) {
			try {
				Customer customer = client.getCustomerById(identifier);
				adapter = new UserAdapter(session, realm, model, customer);
	...
	public int getUsersCount(RealmModel realm) {
		return client.getcustomerCount();
	}

	@Override
	public Stream<UserModel> searchForUserStream(RealmModel realm, String search, Integer firstResult, Integer maxResults) {
		log.debug("searchForUserStream, search={}, first={}, max={}", search, firstResult, maxResults);
		return toUserModelStream(client.getcustomer(search, firstResult, maxResults), realm);
	}

	@Override
	public Stream<UserModel> searchForUserStream(RealmModel realm, Map<String, String> params, Integer firstResult, Integer maxResults) {
		log.debug("searchForUserStream, params={}, first={}, max={}", params, firstResult, maxResults);
		return toUserModelStream(client.getcustomer(null, firstResult, maxResults), realm);
	}

	private Stream<UserModel> toUserModelStream(List<Customer> customer, RealmModel realm) {
		log.debug("Received {} users from provider", customer.size());
		return customer.stream().map(user -> new UserAdapter(session, realm, model, user));
	}
